...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Vite + React | Dev import or client mount | Via adapter contract | Via adapter contract | Via router adapter | Via router adapter | Browser `fetch` | Full | Guaranteed |
| TanStack Router | Adapter | N/A | N/A | Official adapter target | Official adapter target | N/A | Full | Supported with official adapter |
| React Router | Adapter | N/A | N/A | Planned | Planned | N/A | Planned | Manual fallback |
| Next.js | Client-only mount | Client-only later | Client-only later | Manual or adapter later | Client-only later | Browser `fetch` only | Partial later | Manual fallback |
